...
"""SimpleGoalChecker – Python port of nav2_controller::SimpleGoalChecker."""
...
class SimpleGoalChecker(GoalChecker):
    """
    Check if the current pose is within the goal window for x, y and yaw.
    ...
    def is_goal_reached(
        ...
        with self._mutex:
            ...
            if self._check_xy:
                ...
                if dx * dx + dy * dy > self._xy_goal_tolerance_sq:
                    return False
                # We are within the window; if stateful, change the state.
                if self._stateful:
