...
public:
  ...
  virtual void doInitialPerception(geometry_msgs::msg::PoseStamped &)
  {
    ...
    if (exception == "TransformException") {
      ...
      throw nav2_core::ObjectNotValid("ObjectNotValid");
    } else if (exception == "FailedToDetectObject") {
      throw nav2_core::FailedToDetectObject("FailedToDetectObject");
    } else if (exception == "FailedToControl") {
      throw nav2_core::FailedToControl("FailedToControl");
    } else if (exception == "FollowingException") {
      throw nav2_core::FollowingException("FollowingException");
...
{
  auto node = std::make_shared<nav2_following::FollowingServer>();
  node->configure();
  node->activate();
  node->deactivate();
  node->cleanup();
...
{
  auto node = std::make_shared<nav2_following::FollowingServerShim>();
  node->on_configure(rclcpp_lifecycle::State());
